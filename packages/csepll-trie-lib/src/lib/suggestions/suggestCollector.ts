...
// add new code ---start---
// Create locale-aware collators for specific languages
...
export function compSuggestionResults(a: SuggestionResultBase, b: SuggestionResultBase): number {
    ...
    // add new code ---start---
    return aPref - bPref || a.cost - b.cost || a.word.length - b.word.length || compareWords(a.word, b.word);
}

// Compare words using appropriate locale-aware collation
function compareWords(a: string, b: string): number {
    // First, compare base words (without diacritics)
    const baseA = a.normalize('NFD').replaceAll(/\p{Diacritic}/gu, '').toLowerCase();
    const baseB = b.normalize('NFD').replaceAll(/\p{Diacritic}/gu, '').toLowerCase();
    
    if (baseA !== baseB) {
        // If base words are different, use appropriate locale collator
        const hasSpanish = /[áéíóúñÁÉÍÓÚÑ]/.test(a + b);
        const hasFrench = /[àâäçéèêëîïôœùûüÿÀÂÄÇÉÈÊËÎÏÔŒÙÛÜŸ]/.test(a + b);
        
        if (hasSpanish) return esCollator.compare(a, b);
        if (hasFrench) return frCollator.compare(a, b);
        return defaultCollator.compare(a, b);
    }
    
    // Base words are the same, now apply specific ordering rules:
    // 1. Prefer words without diacritics
    const aHasDiacritic = a !== a.normalize('NFD').replaceAll(/\p{Diacritic}/gu, '');
    const bHasDiacritic = b !== b.normalize('NFD').replaceAll(/\p{Diacritic}/gu, '');
    
    if (aHasDiacritic && !bHasDiacritic) return 1;  // b comes first (no diacritic)
    if (!aHasDiacritic && bHasDiacritic) return -1; // a comes first (no diacritic)
    
    // 2. Prefer lowercase over uppercase
    const aIsLower = a === a.toLowerCase();
    const bIsLower = b === b.toLowerCase();
    
    if (aIsLower && !bIsLower) return -1; // a comes first (lowercase)
    if (!aIsLower && bIsLower) return 1;  // b comes first (lowercase)
    
    // 3. For Spanish words with diacritics, use Spanish collator for final ordering
    const hasSpanishDiacritic = /[áéíóúÁÉÍÓÚ]/.test(a + b);
    if (hasSpanishDiacritic) {
        return esCollator.compare(a, b);
    }
    
    // 4. If both have same case and diacritic status, use default collation
    return defaultCollator.compare(a, b);
    // add new code ---end---
}
